#!/usr/bin/env python3
"""
Hippocampus BrainDump — USB Edition GUI
========================================
...
Usage:
  python3 gui_main.py --mode local
  python3 gui_main.py --mode disk --path /media/usb0
"""
...
# PyQt6 .pyd files need Qt6/bin/ DLLs.  On a standard install pip handles
# this via .pth hooks, but embeddable Python skips all of that.
# We must add the Qt6/bin/ directory *before* importing PyQt6.
# ---------------------------------------------------------------------------
if sys.platform == "win32":
    # site-packages could be under the embeddable python/ tree or a venv
    _sp_candidates = [
        Path(sys.prefix) / "Lib" / "site-packages",
        Path(sys.executable).parent / "Lib" / "site-packages",
        Path(__file__).resolve().parent / "python" / "Lib" / "site-packages",
    ]
...
from gui.utils.styles import DARK_STYLE, get_size_str
...
class HippocampusGUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.status_bar.showMessage("就绪 — Hippocampus BrainDump USB Edition")
...
def main():
    parser = argparse.ArgumentParser(description="Hippocampus BrainDump v4.0.0 — USB Edition")
    ...
    if args.mode == "disk" and not args.path:
        # Auto-detect: try to find marker.txt in parent dirs
        script_dir = Path(__file__).parent
        for parent in [script_dir] + list(script_dir.parents):
            if (parent / "marker.txt").exists():
                args.path = str(parent)
                break
        if not args.path:
            print("错误：mode=disk 且未找到 marker.txt，请指定 --path")
            sys.exit(1)
