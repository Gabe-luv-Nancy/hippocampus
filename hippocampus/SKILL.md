...
runtime:
  ...
  instruction: |
    ...
    - User says anything worth remembering ("记住", "save this", "remember that")
    - User asks about past conversations ("之前说过什么", "what did I say")
    - User wants to search memory ("查一下", "search memory")
    ...
    - "把我们今天讨论的记住" → Save to memory
    - "我之前是不是跟你说过..." → Search memory
    - "这个很重要，一会别忘了" → Save to memory
    - "帮我查一下上周那个..." → Recall memory
