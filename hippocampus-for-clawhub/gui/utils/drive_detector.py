#!/usr/bin/env python3
"""
Drive and path detector for Hippocampus.
...
if __name__ == "__main__":
    ...
    print("=== BrainDump Drives ===")
    for d in detector.detect_braindump_drives():
        print(f"  {d.name}: {d.path} (free: {get_size_str(d.free_bytes)})")
    print("=== All Removable Drives ===")
    for d in detector.detect_all_drives():
        print(f"  {d.name}: {d.path} (braindump: {d.is_braindump})")
